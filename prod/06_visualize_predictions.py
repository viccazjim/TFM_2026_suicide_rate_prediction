...
Loads the predictions saved by prod/predict.py together with the
historical df_development.parquet, and generates:
...
TEMPORAL_EXOG_FEATURE = (
    "Alcohol use disorders"  # matches 05_temporal_persistence_check.py's choice
)
TEMPORAL_ORDER = (
    1,
    1,
    1,
)  # see src/timeseries_models.py::train_evaluate_sarimax's docstring —
# (1,1,0) was the original default; notebooks/05b_temporal_persistence_improvements.ipynb found


def _forecast_temporal_model(
    df_history: pd.DataFrame, df_real_world: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    logger.info(
        "Fitting SARIMAX (+%s) per country on Option B training years",
        TEMPORAL_EXOG_FEATURE,
    )
    ...
    if n_converged < n_countries:
        ...
        logger.warning(
            "Only %d/%d countries converged — missing: %s",
            n_converged,
            n_countries,
            sorted(missing),
        )
    ...
    future_block = pd.concat([df_test_B, df_val_B, df_real_world]).sort_values(
        ["Code", "Year"]
    )
...
def run(predictions_path: Path):
    ...
    logger.info(
        "  %d rows, years: %s",
        len(df_predictions_catboost),
        sorted(df_predictions_catboost["Year"].unique().tolist()),
    )

    df_predictions_temporal = _forecast_temporal_model(df_history, df_real_world)
    TABLES_DIR.mkdir(parents=True, exist_ok=True)
    df_predictions_temporal.to_parquet(
        TABLES_DIR / "predictions_temporal.parquet", index=False
    )
    ...
    for code, name in SPOTLIGHT_COUNTRIES:
        ...
        save_figure(
            fig,
            name=f"predictions_trend_{code.lower()}",
            prefix=FIG_PREFIX,
    ...
    for year in sorted(df_predictions_catboost["Year"].unique()):
        ...
        save_figure(
            fig,
            name=f"predictions_by_country_{year}",
            prefix=FIG_PREFIX,
            figures_dir=str(FIGURES_DIR),
        )
    ...
    for code, name in SPOTLIGHT_COUNTRIES:
        ...
        save_figure(
            fig,
            name=f"predictions_comparison_trend_{code.lower()}",
            prefix=FIG_PREFIX,
            figures_dir=str(FIGURES_DIR),
        )
    ...
    for year in sorted(df_predictions_catboost["Year"].unique()):
        ...
        save_figure(
            fig,
            name=f"predictions_comparison_by_country_{year}",
            prefix=FIG_PREFIX,
            figures_dir=str(FIGURES_DIR),
        )
    ...
    for region_name in sorted(set(EU_REGIONS.values())):
        ...
        save_figure(
            fig,
            name=f"predictions_comparison_trend_region_{region_name.lower().replace('/', '_').replace(' ', '_')}",
            prefix=FIG_PREFIX,
            figures_dir=str(FIGURES_DIR),
        )
    ...
    for year in sorted(df_predictions_catboost["Year"].unique()):
        ...
        save_figure(
            fig,
            name=f"predictions_comparison_by_region_{year}",
            prefix=FIG_PREFIX,
            figures_dir=str(FIGURES_DIR),
        )

    logger.info(
        "Done. Figures saved under %s with prefix '%s'.", FIGURES_DIR, FIG_PREFIX
    )
