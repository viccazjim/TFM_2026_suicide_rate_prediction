...
def plot_rmse_comparison(table_test: pd.DataFrame, table_val: pd.DataFrame, label: str):
    ...
    ax.bar(
        x, merged["RMSE_test"], width, label="Test RMSE", color="#4C72B0", alpha=0.85
    )
    ax.bar(
        x + width,
        merged["RMSE_val"],
...
def plot_predictions_trend(df_history, df_predictions, country_code, country_name):
    ...
    (dashed line, connected to the last historical point).
    ...
    if not df_hist.empty and not df_pred.empty:
        ...
        connector_values = [df_hist["Suicide rate"].iloc[-1]] + df_pred[
    ...
    plt.title(
        f"Suicide rate: actual vs predicted — {country_name}",
        fontsize=16,
        fontweight="bold",
        pad=15,
    )
...
def plot_predictions_model_comparison(
    df_history,
    df_predictions_a,
    df_predictions_b,
    model_a_name,
    ...
    instead of one — e.g. the production CatBoost model against the best
    temporal-persistence model from 05_temporal_persistence_check.ipynb.
    Plotting them together shows directly whether the two tell a similar
    story for a given country or diverge, rather than comparing single
    numbers in a table.
    ...
    """
    df_hist = df_history[df_history["Code"] == country_code].sort_values("Year")
    df_pred_a = df_predictions_a[df_predictions_a["Code"] == country_code].sort_values(
        "Year"
    )
    df_pred_b = df_predictions_b[df_predictions_b["Code"] == country_code].sort_values(
        "Year"
    )
...
def plot_predictions_by_country_comparison(
    df_predictions_a, df_predictions_b, model_a_name, model_b_name, year, top_n=None
):
    """
    ...
    matplotlib.figure.Figure
    """
    a = df_predictions_a[df_predictions_a["Year"] == year][
        ["Country", "Predicted suicide rate"]
    ]
    b = df_predictions_b[df_predictions_b["Year"] == year][
        ["Country", "Predicted suicide rate"]
    ]
    merged = a.merge(b, on="Country", suffixes=(f"_{model_a_name}", f"_{model_b_name}"))
    merged = merged.sort_values(
        f"Predicted suicide rate_{model_a_name}", ascending=False
    )
    if top_n:
    ...
    ax.bar(
        x - width / 2,
        merged[f"Predicted suicide rate_{model_a_name}"],
        width,
        label=model_a_name,
        color="#d62728",
    )
    ax.bar(
        x + width / 2,
        merged[f"Predicted suicide rate_{model_b_name}"],
        width,
        label=model_b_name,
        color="#2ca02c",
    )
    ax.set_title(
        f"Predicted suicide rate by country — {year} ({model_a_name} vs {model_b_name})",
        fontweight="bold",
    )
    ax.set_xlabel("Country")
    ax.set_ylabel("Predicted rate per 100,000 inhabitants")
...
def plot_predictions_by_region_comparison(
    df_predictions_a, df_predictions_b, model_a_name, model_b_name, year
):
    """
    ...
    """
    a = (
        df_predictions_a[df_predictions_a["Year"] == year]
        .groupby("Region")["Predicted suicide rate"]
        .mean()
    )
    b = (
        df_predictions_b[df_predictions_b["Year"] == year]
        .groupby("Region")["Predicted suicide rate"]
        .mean()
    )
    merged = (
        pd.DataFrame({model_a_name: a, model_b_name: b})
        .dropna()
        .sort_values(model_a_name, ascending=False)
    )
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(
        x - width / 2, merged[model_a_name], width, label=model_a_name, color="#d62728"
    )
    ax.bar(
        x + width / 2, merged[model_b_name], width, label=model_b_name, color="#2ca02c"
    )
    ax.set_title(
        f"Predicted suicide rate by EU region — {year} ({model_a_name} vs {model_b_name})",
        fontweight="bold",
    )
...
def plot_predictions_trend_by_region(
    df_history,
    df_predictions_a,
    df_predictions_b,
    model_a_name,
    ...
    """
    hist = (
        df_history[df_history["Region"] == region_name]
        .groupby("Year")["Suicide rate"]
        .mean()
        .reset_index()
    )
    pred_a = (
        df_predictions_a[df_predictions_a["Region"] == region_name]
        .groupby("Year")["Predicted suicide rate"]
        .mean()
        .reset_index()
    )
    pred_b = (
        df_predictions_b[df_predictions_b["Region"] == region_name]
        .groupby("Year")["Predicted suicide rate"]
        .mean()
        .reset_index()
    )
    ...
    for pred, color, label in [
        (pred_a, "#d62728", model_a_name),
        (pred_b, "#2ca02c", model_b_name),
    ]:
        if hist.empty or pred.empty:
            continue
        connector_years = [hist["Year"].iloc[-1]] + pred["Year"].tolist()
        connector_values = [hist["Suicide rate"].iloc[-1]] + pred[
            "Predicted suicide rate"
        ].tolist()
        plt.plot(
            connector_years,
    ...
    plt.title(
        f"Suicide rate: actual vs predicted — {region_name} (region average)",
        fontsize=16,
        fontweight="bold",
        pad=15,
    )
...
def plot_kmeans_elbow_silhouette(sweep_df: pd.DataFrame):
    ...
    axes[1].axvline(
        best_k, color="gray", linestyle="--", alpha=0.7, label=f"Best: k={int(best_k)}"
    )
...
def plot_cluster_vs_region_pca(
    X_scaled: pd.DataFrame, cluster_labels, region_labels, country_codes
):
    """
