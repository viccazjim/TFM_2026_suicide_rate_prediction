...
prediction of the target; this one never touches "Suicide rate" as
something to predict — only as one more property of a country to
describe it by, alongside socioeconomic indicators.
"""
...
def aggregate_country_features(
    ...
    different clusters across years, which doesn't make sense for
    validating a label (Region) that doesn't change over time.

    For each country: the mean of every predictor and of the target
    (the country's typical level, 2000-2021), plus the target's linear
    trend over time (slope from an OLS fit of target ~ year) — level
    alone would treat a country with a flat high suicide rate the same
    as one climbing rapidly toward it.

    Parameters
    ----------
    df : pd.DataFrame
        Country-year panel, e.g. df_development.
    predictor_features : list[str]
        Predictor columns to average (does not include target or year).
    target : str, default "Suicide rate"
        Column to compute both mean and trend for.
    ...
    pd.DataFrame
        ...
        predictor and the target, and "{target}_trend" (slope, target
        units per year — positive means rising).
    """
    level = df.groupby(id_col)[predictor_features + [target]].mean()
